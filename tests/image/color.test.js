...
Object.keys(colorArr).forEach((k) => {
  test(`test overlay-${colorArr[k]}`, () => {
    const key = ts(`overlay-${colorArr[k]}`);
    const v = colorbase[colorArr[k]];
    const color =
      v === 'transparent'
        ? { overlayColor: 'transparent' }
        : { overlayColor: addOpacity(v, 1) };
    expect(key).toEqual(color);
  });
});

test(`test overlay-blue-c1`, () => {
  const key = ts(`overlay-blue-c1-50`);
  const v = '#E9F0FE';
  const color = { overlayColor: addOpacity(v, 0.5) };
  expect(key).toEqual(color);
});
