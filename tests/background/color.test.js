...
Object.keys(colorArr).forEach((k) => {
  test(`test bg-${colorArr[k]}`, () => {
    const key = ts(`bg-${colorArr[k]}`);
    const v = colorbase[colorArr[k]];
    const color =
      v === 'transparent'
        ? { backgroundColor: 'transparent' }
        : { backgroundColor: addOpacity(v, 1) };
    expect(key).toEqual(color);
  });
});

test(`test bg-blue-c1-50`, () => {
  const key = ts(`bg-blue-c1-50`);
  const v = '#E9F0FE';
  const color = { backgroundColor: addOpacity(v, 0.5) };
  expect(key).toEqual(color);
});
